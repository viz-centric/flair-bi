...
public class PropertyTypeResourceIntTest extends AbstractIntegrationTest {
	...
	public void getAllPropertyTypes() throws Exception {
		...
		restPropertyTypeMockMvc.perform(get("/api/propertyTypes?sort=id,desc&page=0&size=1")).andExpect(status().isOk())
	...
	public void getPropertyTypes() throws Exception {
		PropertyType propertyType = propertyTypeService.save(createEntity());
		restPropertyTypeMockMvc.perform(get("/api/propertyTypes/{id}", propertyType.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void createPropertyTypes() throws Exception {
		restPropertyTypeMockMvc
				.perform(post("/api/propertyTypes").contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void updatePropertyTypes() throws Exception {
		PropertyType propertyType = propertyTypeService.save(createEntity());
		restPropertyTypeMockMvc
				.perform(put("/api/propertyTypes").contentType(TestUtil.APPLICATION_JSON_UTF8)
						.content(TestUtil.convertObjectToJsonBytes(updateEntity(propertyType.getId()))))
				.andExpect(status().isOk()).andExpect(jsonPath("$.name").value(UPDATED_NAME))
