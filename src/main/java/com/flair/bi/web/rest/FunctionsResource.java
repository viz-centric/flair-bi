...
public class FunctionsResource {
	 ...
	 *         functionsDTO, or with status 400 (Bad Request) if the functions has
	 *         already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/functions")
	@Timed
	public ResponseEntity<FunctionsDTO> createFunctions(@RequestBody FunctionsDTO functionsDTO)
			throws URISyntaxException {
		log.debug("REST request to save Functions : {}", functionsDTO);
		if (functionsDTO.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert("functions", "idexists", "A new functions cannot already have an ID"))
					.body(null);
		}
		FunctionsDTO result = functionsService.save(functionsDTO);
		return ResponseEntity.created(new URI("/api/functions/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("functions", result.getId().toString())).body(result);
	}
	 ...
	 *         functionsDTO, or with status 400 (Bad Request) if the functionsDTO is
	 *         not valid, or with status 500 (Internal Server Error) if the
	 *         functionsDTO couldnt be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/functions")
	@Timed
	public ResponseEntity<FunctionsDTO> updateFunctions(@RequestBody FunctionsDTO functionsDTO)
			throws URISyntaxException {
		...
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityUpdateAlert("functions", functionsDTO.getId().toString())).body(result);
	}

	/**
	 * GET /functions : get all the functions.
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of functions in
	 *         body
	 */
	 ...
	 *         functionsDTO, or with status 404 (Not Found)
	 */
	...
	public ResponseEntity<FunctionsDTO> getFunctions(@PathVariable Long id) {
		...
		return Optional.ofNullable(functionsDTO).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
