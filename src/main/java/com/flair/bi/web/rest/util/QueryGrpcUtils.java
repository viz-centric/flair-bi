...
public final class QueryGrpcUtils {
	...
	public static Connection toProtoConnection(ConnectionDTO connection) {
		return Optional.ofNullable(connection).map(c -> {
			Connection.Builder builder = Connection.newBuilder()
					.setConnectionPassword(orEmpty(c.getConnectionPassword()))
					.setConnectionUsername(c.getConnectionUsername()).setConnectionType(c.getConnectionTypeId())
					.setName(c.getName()).putAllDetails(c.getDetails());
	...
	private static List<HavingDTO> getListHavingDTO(List<Query.HavingHolder> havingList) {
		return havingList.stream()
				.map(h -> HavingDTO.builder().feature(toFieldDTO(h.getFeature()))
						...
						.operation(toQueryDTO(h.getOperation())).build())
	...
	private static Operation createOperation(String operationJson) {
		return JacksonUtil.fromString(operationJson, Operation.class);
	}
