...
public class VisualMetadataResourceTest extends AbstractIntegrationTest {
    ...
    public void validateVisualMetadataRequestValidationFails() {
        ...
        when(visualMetadataValidationService.validate(eq(1L),
            eq(new QueryDTO()),
            eq("visual id"),
            eq(new LikeConditionExpression()),
            eq("user id")))
            .thenReturn(dto);

        HashMap result = restTemplate
            .withBasicAuth("flairuser", "flairpass")
            .postForObject(getUrl() + "/api/visualmetadata/validate", new ValidateVisualMetadataDTO(), HashMap.class);
    ...
    public void validateVisualMetadataValidationSucceeds() {
        ...
        when(visualMetadataValidationService.validate(
            eq(datasourceId),
            any(QueryDTO.class),
            eq(visualId),
            any(ConditionExpression.class),
            eq("flairuser")))
            .thenReturn(dto);

        QueryValidationResponseDTO result = restTemplate
            .withBasicAuth("flairuser", "flairpass")
    ...
    public void createVisualMetadata() {
        ...
        ResponseEntity<VisualMetadata> result = restTemplate
                .withBasicAuth("flairuser", "flairpass")
                .postForEntity(getUrl() + "/api/visualmetadata",
                        visualMetadata,
    ...
    public void updateVisualMetadataCreatesMetadata() {
        ...
        ResponseEntity<VisualMetadata> result = restTemplate
                .withBasicAuth("flairuser", "flairpass")
                .exchange(getUrl() + "/api/visualmetadata",
                        HttpMethod.PUT,
                        new HttpEntity<>(visualMetadata),
                        VisualMetadata.class);
    ...
    public void getAllVisualMetadata() {
        ...
        ResponseEntity<VisualMetadata[]> result = restTemplate
                .withBasicAuth("flairuser", "flairpass")
                .exchange(getUrl() + "/api/visualmetadata?views=17",
                        HttpMethod.GET,
    ...
    public void getVisualMetadata() {
        ...
        ResponseEntity<VisualMetadata> result = restTemplate
                .withBasicAuth("flairuser", "flairpass")
                .exchange(getUrl() + "/api/visualmetadata/18",
                        HttpMethod.GET,
                        new HttpEntity<>(new LinkedMultiValueMap<>()),
                        VisualMetadata.class);
