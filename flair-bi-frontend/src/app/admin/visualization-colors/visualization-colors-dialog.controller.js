...
function VisualizationColorsDialogController($timeout, $scope, $stateParams, $uibModalInstance, entity, VisualizationColors, $translate, $rootScope) {
    ...
    function onSaveSuccess(result) {
        onSave(result);
        var info = {
            text: $translate.instant('flairbiApp.visualizationColors.created', {param: result.id}),
            title: "Saved"
        }
    ...
    function onUpdateSuccess(result) {
        onSave(result);
        var info = {
            text: $translate.instant('flairbiApp.visualizationColors.updated', {param: result.id}),
            title: "Updated"
        }
        $rootScope.showSuccessToast(info);
    }
