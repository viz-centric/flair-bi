...
public class HierarchyResource {
	 ...
	 *         hierarchy, or with status 400 (Bad Request) if the hierarchy is not
	 *         valid, or with status 404 (Not Found) if hierarchy with given id does
	 *         not exists, or with status 500 (Internal Server Error) if the
	 *         hierarchy couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/hierarchies")
	@Timed
	public ResponseEntity<Hierarchy> updateHierarchy(@Valid @RequestBody HierarchyDTO hierarchy)
		...
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityUpdateAlert("hierarchies", hierarchy.getId().toString())).body(result);
	}
	 ...
	 *         hierarchy, or with status 400 (Bad Request) if the hierarchy has
	 *         already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<Hierarchy> createHierarchy(@Valid @RequestBody Hierarchy hierarchy)
		...
		if (hierarchy.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("hierarchies", "idexists",
					"A new features cannot already have an ID")).body(null);
		}
		Hierarchy result = hierarchyService.save(hierarchy);
		return ResponseEntity.created(new URI("/api/hierarchies/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("hierarchies", result.getId().toString())).body(result);
	}
