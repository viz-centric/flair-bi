...
public class DatasourceResourceIntTest extends AbstractIntegrationTest {
        ...
        public void createDatasources() throws Exception {
                ...
                restDatasourcesMockMvc
                                .perform(post("/api/datasources").contentType(TestUtil.APPLICATION_JSON_UTF8)
        ...
        public void getAllDatasources() throws Exception {
                ...
                restDatasourcesMockMvc.perform(get("/api/datasources")).andExpect(status().isOk())
                                ...
                                .andExpect(jsonPath("$.[*].lastUpdated")
                                                .value(hasItem(sameInstant(DEFAULT_LAST_UPDATED))))
                                .andExpect(jsonPath("$.[*].connectionName")
                                                .value(hasItem(DEFAULT_CONNECTION_NAME.toString())))
        ...
        public void updateDatasources() throws Exception {
                ...
                Datasource updatedDatasource = datasourceRepository.getOne(datasource.getId());
                updatedDatasource.name(UPDATED_NAME).lastUpdated(UPDATED_LAST_UPDATED)
                                .connectionName(UPDATED_CONNECTION_NAME).queryPath(UPDATED_QUERY_PATH);

                restDatasourcesMockMvc
                                .perform(put("/api/datasources").contentType(TestUtil.APPLICATION_JSON_UTF8)
        ...
        public void listTables() throws Exception {
                ...
                when(grpcConnectionService.listTables(eq("connection link"), eq("some table"),
                                isNull(ConnectionDTO.class), eq(10))).thenReturn(new ListTablesResponseDTO().setTables(
                                                Arrays.asList(new ListTablesResponseDTO.Table("table1", "sql1"))));
                ...
                restDatasourcesMockMvc
                                ...
                                .andExpect(status().isOk())
                                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
                                .andExpect(jsonPath("$.tableNames[0].name").value("table1"))
                                .andExpect(jsonPath("$.tableNames[0].sql").value("sql1"));
        }
