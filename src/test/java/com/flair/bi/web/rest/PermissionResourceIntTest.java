...
public class PermissionResourceIntTest extends AbstractIntegrationTest {
	...
	@Before
	public void setup() {
		PermissionResource permissionResource = new PermissionResource(userGroupService, userService,
	...
	public void addPermissionToUserGroup() throws Exception {
		restPermissionMockMvc
				.perform(put("/api/permissions/{permission}/userGroups/{userGroup}", PERMISSION_ID, ROLE_DEVELOPMENT))
	...
	public void removePermissionFromUserGroup() throws Exception {
		restPermissionMockMvc
				.perform(
						delete("/api/permissions/{permission}/userGroups/{userGroup}", PERMISSION_ID, ROLE_DEVELOPMENT))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.name").value(ROLE_DEVELOPMENT));
	...
	public void addPermission() throws Exception {
		...
		restPermissionMockMvc
				.perform(put("/api/permissions/{id}/users/{login:" + Constants.LOGIN_REGEX + "}", PERMISSION_ID,
						user.getLogin()))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void removePermission() throws Exception {
		User user = UserResourceIntTest.createEntity(userService);
		restPermissionMockMvc
				.perform(delete("/api/permissions/{id}/users/{login:" + Constants.LOGIN_REGEX + "}", PERMISSION_ID,
						user.getLogin()))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.login").value(user.getLogin()))
				.andExpect(jsonPath("$.email").value(user.getEmail()))
