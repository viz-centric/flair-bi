 ...
 * between User and Permission, and send everything to the client side: there
...
public class UserResource {
	...
	/**
	 * GET /users/:login/dashboardsMetadata : get permission information for
	 * dashboards for certain user
	 *
	 * @param login    usr login information
	 * @param pageable pagination parameters
	 * @throws URISyntaxException exception
	 * @return the ResponseEntity with status 200(OK), or status 404 (Not Found) if
	 *         user with provided login does not exist
	 */
	@GetMapping("/users/{login:" + Constants.LOGIN_REGEX + "}/dashboardPermissions")
	@Timed
	public ResponseEntity<List<GranteePermissionReport<User>>> getDashboardPermissionMetadataUser(
			@PathVariable String login, @ApiParam Pageable pageable) throws URISyntaxException {
		...
		List<GranteePermissionReport<User>> body = dashboardPage.getContent().stream()
				.map(x -> x.getGranteePermissionReport(user)).collect(Collectors.toList());
	...
	@Timed
	public ResponseEntity<List<GranteePermissionReport<User>>> getViewPermissionMetadataUser(@PathVariable String login,
			@PathVariable Long id) {
		final User user = userService.getUserWithAuthoritiesByLogin(login).orElseThrow(
				() -> new EntityNotFoundException(String.format("User with login: %s was not found", login)));

		List<GranteePermissionReport<User>> body = viewService.findByDashboardId(id).stream()
				.map(x -> x.getGranteePermissionReport(user)).collect(Collectors.toList());
	...
	@Timed
	public ResponseEntity<Void> changePermissions(@PathVariable String login,
	...
	@Timed
	public ResponseEntity<List<ManagedUserVM>> getSearchedUsers(@ApiParam Pageable pageable,
			@QuerydslPredicate(root = User.class) Predicate predicate) throws URISyntaxException {
