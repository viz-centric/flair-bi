...
public class AccessControlManagerImplTest {
	...
	public void removePermissionTest() {
		...
		Assert.assertTrue(permissionRepository.findById(new PermissionKey("a", Action.READ, "a")).isEmpty());
		Assert.assertTrue(permissionRepository.findById(new PermissionKey("a", Action.WRITE, "a")).isEmpty());
	}
	...
	public void grantRevokeUserGroupAccessCheck() {
		...
		Assert.assertTrue(
				userGroupRepository.getOne("test").getPermissions().contains(new Permission("test", Action.READ, "b")));
