...
public class QueryValidationServiceTest {
	...
	public void validateQuerySucceeds() {
		...
		query.setHaving(asList(new HavingDTO(new FieldDTO("name", "MAX", "namemax"), HavingDTO.ComparatorType.GT,
				new ScalarOperation("190"))));
		query.setOrders(asList(new SortDTO(new FieldDTO("order"), SortDTO.Direction.ASC)));
		...
		query.setConditionExpressions(
				asList(new ConditionExpressionDTO(ConditionExpressionDTO.SourceType.FILTER, conditionExpression)));
		...
		QueryValidationParams validationParams = QueryValidationParams.builder()
				.validationType(QueryValidationType.REQUIRED_FIELDS).features(features).build();
	...
	public void validateInvalidCondition() {
		...
		assertEquals(asList(QueryValidationError.of("order_date", "RequiredConditionFeatureMissing")),
				result.getErrors());
	}
}
