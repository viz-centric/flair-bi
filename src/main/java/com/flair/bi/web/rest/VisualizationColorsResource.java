...
public class VisualizationColorsResource {
	 ...
	 *         visualizationColorsDTO, or with status 400 (Bad Request) if the
	 *         visualizationColors has already an ID
	...
	public ResponseEntity<VisualizationColorsDTO> createVisualizationColors(
			@Valid @RequestBody VisualizationColorsDTO visualizationColorsDTO) throws URISyntaxException {
		log.debug("REST request to save VisualizationColors : {}", visualizationColorsDTO);
		if (visualizationColorsDTO.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("visualizationColors", "idexists",
					"A new visualizationColors cannot already have an ID")).body(null);
		}
	 ...
	 *         visualizationColorsDTO, or with status 400 (Bad Request) if the
	 *         visualizationColorsDTO is not valid, or with status 500 (Internal
	 *         Server Error) if the visualizationColorsDTO couldnt be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/visualization-colors")
	@Timed
	@PreAuthorize("@accessControlManager.hasAccess('VISUALIZATION_COLORS', 'UPDATE','APPLICATION')")
	public ResponseEntity<VisualizationColorsDTO> updateVisualizationColors(
			@Valid @RequestBody VisualizationColorsDTO visualizationColorsDTO) throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(
				HeaderUtil.createEntityUpdateAlert("visualizationColors", visualizationColorsDTO.getId().toString()))
				.body(result);
	}
	 ...
	 *         visualizationColorsDTO, or with status 404 (Not Found)
	 */
	...
	public ResponseEntity<VisualizationColorsDTO> getVisualizationColors(@PathVariable Long id) {
		...
		return Optional.ofNullable(visualizationColorsDTO).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
	...
	public ResponseEntity<Void> deleteVisualizationColors(@PathVariable Long id) {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("visualizationColors", id.toString()))
				.build();
	}
