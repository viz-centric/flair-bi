...
public class FeatureBookmarkResourceIntTest extends AbstractIntegrationTest {
	...
	public static FeatureBookmark createEntity(EntityManager em, User user) {
		FeatureBookmark featureBookmark = new FeatureBookmark().name(DEFAULT_NAME);
		// Add required entity
		em.persist(user);
		em.flush();
	...
	public void createFeatureBookmark() throws Exception {
		...
		restFeatureBookmarkMockMvc.perform(post("/api/feature-bookmarks").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(featureBookmark))).andExpect(status().isCreated());
	...
	public void createFeatureBookmarkWithExistingId() throws Exception {
		...
		restFeatureBookmarkMockMvc
				.perform(post("/api/feature-bookmarks").contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void getAllFeatureBookmarks() throws Exception {
		...
		restFeatureBookmarkMockMvc.perform(get("/api/feature-bookmarks")).andExpect(status().isOk())
	...
	public void updateFeatureBookmark() throws Exception {
		...
		FeatureBookmark updatedFeatureBookmark = featureBookmarkRepository.getOne(featureBookmark.getId());
		updatedFeatureBookmark.name(UPDATED_NAME);

		restFeatureBookmarkMockMvc.perform(put("/api/feature-bookmarks").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedFeatureBookmark))).andExpect(status().isOk());
	...
	public void deleteFeatureBookmark() throws Exception {
		...
		// Get the featureBookmark
		restFeatureBookmarkMockMvc.perform(
				delete("/api/feature-bookmarks/{id}", featureBookmark.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
