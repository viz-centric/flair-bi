...
public class QueryValidationService {
	...
	public QueryValidationResult validate(QueryDTO queryDTO, QueryValidationParams params) {
		...
		if (!selectFieldsValidationResult.isEmpty()) {
			return QueryValidationResult.builder().errors(selectFieldsValidationResult)
					.group(QueryValidationResult.Group.SELECT).build();
		}

		List<QueryValidationError> groupByValidationResult = validateFields(queryDTO.getGroupBy());
		if (!groupByValidationResult.isEmpty()) {
			return QueryValidationResult.builder().errors(groupByValidationResult)
					.group(QueryValidationResult.Group.GROUP_BY).build();
		}

		List<QueryValidationError> orderByValidationResult = validateOrders(queryDTO.getOrders());
		if (!orderByValidationResult.isEmpty()) {
			return QueryValidationResult.builder().errors(orderByValidationResult)
					.group(QueryValidationResult.Group.ORDER_BY).build();
		}

		List<QueryValidationError> havingResult = validateHavings(queryDTO.getHaving());
		if (!havingResult.isEmpty()) {
			return QueryValidationResult.builder().errors(havingResult).group(QueryValidationResult.Group.HAVING)
					.build();
		}
		...
		if (params.getValidationType() == QueryValidationType.REQUIRED_FIELDS) {
			...
			if (!conditionsResult.isEmpty()) {
				return QueryValidationResult.builder().errors(conditionsResult)
						.group(QueryValidationResult.Group.CONDITIONS).build();
			}
		}

		return QueryValidationResult.builder().build();
	}

	private List<QueryValidationError> validateConditions(List<ConditionExpressionDTO> conditionExpressions,
			Map<String, Feature> features) {
		Set<String> featureNamesWithDateFilterEnabled = new HashSet<>(getRequiredByConditionFeatureNames(features));
		conditionExpressions.forEach(
				conditionExpressionDTO -> validateCondition(conditionExpressionDTO.getConditionExpression(), features)
	...
	private Set<String> getRequiredByConditionFeatureNames(Map<String, Feature> features) {
		return features.values().stream().filter(feature -> feature.getDateFilter() == DateFilterType.ENABLED)
				.map(feature -> feature.getName()).collect(toSet());
	}
	...
	private Set<String> getConditionFeatureNames(ConditionExpression conditionExpression) {
		...
		if (conditionExpression instanceof OrConditionExpression) {
			...
			if (compareConditionExpression.getFeatureProperty() != null
					&& compareConditionExpression.getFeatureProperty().getProperty() != null) {
	...
	private List<QueryValidationError> validateOrders(List<SortDTO> orders) {
		return orders.stream().map(order -> validateOrder(order)).filter(optional -> optional.isPresent())
				.map(optional -> optional.get()).collect(toList());
	}
	...
	private List<QueryValidationError> validateHavings(List<HavingDTO> havings) {
		return havings.stream().map(having -> validateHaving(having)).filter(optional -> optional.isPresent())
				.map(optional -> optional.get()).collect(toList());
	}
	...
	private Optional<QueryValidationError> validateHaving(HavingDTO having) {
		...
		if (having.getOperation() == null) {
			return Optional.ofNullable(QueryValidationError.of("", "HavingValueInvalid"));
		}
		return Optional.empty();
	}

	private List<QueryValidationError> validateFields(List<FieldDTO> fields) {
		return fields.stream().map(field -> validateField(field)).filter(field -> field.isPresent())
				.map(field -> field.get()).collect(toList());
	}

	private Optional<QueryValidationError> validateField(FieldDTO field) {
		if (!StringUtils.isEmpty(field.getAggregation()) && !FIELD_REGEX.matcher(field.getAggregation()).matches()) {
			return Optional.ofNullable(QueryValidationError.of(field.getAggregation(), "FieldAggregationInvalid"));
		}
		if (!StringUtils.isEmpty(field.getName()) && !FIELD_REGEX.matcher(field.getName()).matches()) {
			return Optional.ofNullable(QueryValidationError.of(field.getName(), "FieldNameInvalid"));
		}
		if (!StringUtils.isEmpty(field.getAlias()) && !FIELD_REGEX.matcher(field.getAlias()).matches()) {
			return Optional.ofNullable(QueryValidationError.of(field.getAlias(), "FieldAliasInvalid"));
		}
		return Optional.empty();
	}
}
