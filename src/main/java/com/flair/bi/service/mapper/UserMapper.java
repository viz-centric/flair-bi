...
public interface UserMapper {
	...
	default Set<String> stringsFromPermissions(Set<Permission> permissions) {
		return permissions.stream().map(Permission::getStringValue).collect(Collectors.toSet());
	}

	default Set<String> stringsFromUserGroups(Set<UserGroup> userGroups) {
		return userGroups.stream().map(UserGroup::getName).collect(Collectors.toSet());
	}

	default Set<Permission> permissionsFromStrings(Set<String> strings) {
		return strings.stream().map(Permission::fromStringValue).collect(Collectors.toSet());
	}
