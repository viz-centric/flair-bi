(function () {
    ...
    ExplorationNavController.$inject = ['$scope', '$state', 'Auth', 'Principal', 'ProfileService', 'LoginService',
        'Visualizations', 'Measures', 'Dimensions', '$rootScope', 'Datasources', '$transitions'];

    function ExplorationNavController($scope, $state, Auth, Principal,
        ProfileService, LoginService, Visualizations, Measures, Dimensions, $rootScope,
        Datasources, $transitions) {
        var vm = this;
        initializeMetisMenu();
        getAccount();
        vm.account = null;
