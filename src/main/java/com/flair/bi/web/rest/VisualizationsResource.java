...
public class VisualizationsResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new visualization, or with status 400 (Bad Request) if the visualization has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/visualizations")
    @Timed
    @PreAuthorize("@accessControlManager.hasAccess('VISUALIZATIONS', 'WRITE', 'APPLICATION')")
    public ResponseEntity<Visualization> createVisualizations(@Valid @RequestBody Visualization visualization) throws URISyntaxException {
        log.debug("REST request to save Visualization : {}", visualization);
        if (visualization.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("visualization", "idexists", "A new visualization cannot already have an ID")).body(null);
        }
        Visualization result = visualizationService.save(visualization);
        return ResponseEntity.created(new URI("/api/visualization/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("visualization", result.getId().toString()))
            .body(result);
    }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated visualization,
     * or with status 400 (Bad Request) if the visualization is not valid,
     * or with status 500 (Internal Server Error) if the visualization couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/visualizations")
    @Timed
    @PreAuthorize("@accessControlManager.hasAccess('VISUALIZATIONS','UPDATE', 'APPLICATION')")
    public ResponseEntity<Visualization> updateVisualizations(@Valid @RequestBody Visualization visualization) throws URISyntaxException {
     ...
     *
     * @return the ResponseEntity with status 200 (OK) and the list of visualizations in body
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the visualizations, or with status 404 (Not Found)
     */
    ...
    public ResponseEntity<Void> deleteVisualizations(@PathVariable Long id) {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("visualizations", id.toString())).build();
    }
    ...
    @Timed
    @PreAuthorize("@accessControlManager.hasAccess('VISUALIZATIONS', 'UPDATE', 'APPLICATION')")
    public ResponseEntity<Visualization> createNewFieldType(@PathVariable Long id, @Valid @RequestBody FieldType fieldType) throws URISyntaxException {
        if (fieldType.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("fieldTypes", "idexists", "A new field type cannot already have an ID")).body(null);
        }
        Visualization result = visualizationService.saveFieldType(id, fieldType);
        return ResponseEntity.created(new URI("/api/visualizations/" + id + "/fieldTypes/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("fieldType", result.getId().toString()))
            .body(result);
    }
    ...
    @Timed
    @PreAuthorize("@accessControlManager.hasAccess('VISUALIZATIONS', 'DELETE', 'APPLICATION')")
    public ResponseEntity<Visualization> deleteFieldType(@PathVariable Long id, @PathVariable Long fieldTypeId) {
        final Visualization body = visualizationService.deleteFieldType(id, fieldTypeId);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("fieldType", fieldTypeId.toString())).body(body);
    }

    @PostMapping("/visualizations/{id}/propertyTypes")
    @Timed
    @PreAuthorize("@accessControlManager.hasAccess('VISUALIZATIONS', 'UPDATE', 'APPLICATION')")
    public ResponseEntity<Visualization> assignPropertyType(@PathVariable Long id, @RequestBody IdentifierDTO<Long> propertyType) {
    ...
    @PreAuthorize("@accessControlManager.hasAccess('VISUALIZATIONS', 'UPDATE', 'APPLICATION')")
    public ResponseEntity<Visualization> removePropertyType(@PathVariable Long id, @PathVariable Long propertyTypeId) {
