...
public class AlertsResourceTest extends AbstractIntegrationTest {
	...
	public void getReleaseAlerts() throws Exception {
		when(releaseRequestService.getTodaysReleasedAlerts(eq(0)))
				.thenReturn(Arrays.asList(new ReleasesAlertsDTO("myRelease", "myComment", "myName",
						Timestamp.from(Instant.ofEpochSecond(1550510210)))));

		restMvc.perform(get("/api/release-alerts/1/0").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	public void getAllReleaseAlerts() throws Exception {
		restMvc.perform(get("/api/release-all-alerts").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[0].id").value(is(1))).andExpect(jsonPath("$.[0].name").value(is("Today")))
				.andExpect(jsonPath("$.[0].alerts.length()").value(is(0)))
				.andExpect(jsonPath("$.[0].count").value(is(0))).andExpect(jsonPath("$.[1].id").value(is(2)))
				.andExpect(jsonPath("$.[1].name").value(is("Yesterday")))
				.andExpect(jsonPath("$.[1].alerts.length()").value(is(0)))
				.andExpect(jsonPath("$.[1].count").value(is(0))).andExpect(jsonPath("$.[2].id").value(is(3)))
				.andExpect(jsonPath("$.[2].name").value(is("This Week")))
				.andExpect(jsonPath("$.[2].alerts.length()").value(is(0)))
				.andExpect(jsonPath("$.[2].count").value(is(0))).andExpect(jsonPath("$.[3].id").value(is(4)))
				.andExpect(jsonPath("$.[3].name").value(is("Last Week")))
				.andExpect(jsonPath("$.[3].alerts.length()").value(is(0)))
				.andExpect(jsonPath("$.[3].count").value(is(0))).andExpect(jsonPath("$.[4].id").value(is(5)))
