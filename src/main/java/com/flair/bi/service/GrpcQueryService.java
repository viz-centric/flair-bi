...
public class GrpcQueryService {
	...
	public void sendGetDataStream(SendGetDataDTO sendGetDataDTO) throws InterruptedException {
		...
		DatasourceConstraint constraint = datasourceConstraintService
				.findByUserAndDatasource(sendGetDataDTO.getUserId(), datasource.getId());
		...
		if (sendGetDataDTO.getVisualMetadata() != null && sendGetDataDTO.getType() == null) {
			callGrpcBiDirectionalAndPushInSocket(datasource, sendGetDataDTO.getVisualMetadata().getId(),
					"vizualization", sendGetDataDTO);
		} else if (sendGetDataDTO.getVisualMetadata() != null && sendGetDataDTO.getType().equals("share-link")) {
			callGrpcBiDirectionalAndPushInSocket(datasource, sendGetDataDTO.getVisualMetadata().getId(), "share-link",
					sendGetDataDTO);
		} else {
			callGrpcBiDirectionalAndPushInSocket(datasource, sendGetDataDTO.getVisualMetadataId(), "filters",
					sendGetDataDTO);
		}
	}
	...
	private void callGrpcBiDirectionalAndPushInSocket(Datasource datasource, String vId, String request,
		...
		try {
			query = queryTransformerService.toQuery(queryDTO,
					QueryTransformerParams.builder().datasourceId(datasource.getId())
							...
							.validationType(sendGetDataDTO.getValidationType()).vId(vId).userId(userId).build());
		} catch (QueryTransformerException e) {
			...
			fbEngineWebSocketService.pushGRPCMetaDataError(userId, Status.FAILED_PRECONDITION,
					ImmutableMap.of("group", e.getValidationResult().getGroup().name().toLowerCase(), "error",
							e.getValidationResult().getErrors().get(0).getError(), "value",
							e.getValidationResult().getErrors().get(0).getValue()));
			return;
		}
