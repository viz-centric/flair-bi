...
public class UserResourceIntTest extends AbstractIntegrationTest {
	...
	public void setup() {
		...
		UserResource userResource = new UserResource(userRepository, mailService, userService, dashboardService,
				viewService, accessControlManager);
	...
	public void testGetExistingUser() throws Exception {
		...
		restUserMockMvc.perform(get("/api/users/admin").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.login").value("admin")).andExpect(jsonPath("$.email").value("test@gmail.com"))
	...
	public void createNewUser() throws Exception {
		restUserMockMvc
				...
				.andExpect(jsonPath("$.login").value(LOGIN)).andExpect(jsonPath("$.email").value(EMAIL))
				.andExpect(jsonPath("$.firstName").value(FIRST_NAME)).andExpect(jsonPath("$.lastName").value(LAST_NAME))
	...
	public void updateUser() throws Exception {
		...
		restUserMockMvc
				...
				.andExpect(jsonPath("$.login").value(UPDATED_LOGIN)).andExpect(jsonPath("$.email").value(UPDATED_EMAIL))
	...
	public void getAllUsers() throws Exception {
		...
		restUserMockMvc.perform(get("/api/users?sort=id,desc&page=0&size=5")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getDashboardPermissionMetadataUser() throws Exception {
		...
		restUserMockMvc
				.perform(get("/api/users/{login:" + Constants.LOGIN_REGEX
						+ "}/dashboardPermissions?sort=id,desc&page=0&size=5", LOGIN))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
