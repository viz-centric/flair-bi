...
public class PropertyTypeResource {
	 ...
	 * @return the ResponseEntity with status 200 (OK) and the list of property
	 *         types in body
	 */
	@GetMapping("/propertyTypes")
	@Timed
	public ResponseEntity<List<PropertyTypeDTO>> getAllPropertyTypes(@ApiParam Pageable pageable)
	 ...
	 *         propertyType, or with status 404 (Not Found)
	 */
	...
	public ResponseEntity<PropertyType> getPropertyType(@PathVariable Long id) {
		...
		return ResponseUtil.wrapOrNotFound(Optional.ofNullable(propertyTypeService.findById(id)));
	}
	 ...
	 *         propertyType, or with status 400 (Bad Request) if the propertyType
	 *         has already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/propertyTypes")
	@Timed
	public ResponseEntity<PropertyType> insertPropertyType(@Valid @RequestBody PropertyType propertyType)
			throws URISyntaxException {
		log.debug("REST request to insert Property type : {}", propertyType);
		if (propertyType.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("propertyTypes", "idexists",
					"A new property type cannot already have an ID")).body(null);
		}
		...
		return ResponseEntity.created(new URI("/api/propertyTypes/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("propertyTypes", result.getId().toString())).body(result);
	}
	 ...
	 *         property type, or with status 400 (Bad Request) if the property type
	 *         is not valid, or with status 500 (Internal Server Error) if the
	 *         property type could not be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/propertyTypes")
	@Timed
	public ResponseEntity<PropertyType> updateDatasources(@Valid @RequestBody PropertyType propertyType)
			throws URISyntaxException {
