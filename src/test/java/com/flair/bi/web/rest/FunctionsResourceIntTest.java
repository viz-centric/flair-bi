...
public class FunctionsResourceIntTest extends AbstractIntegrationTest {
	...
	public static Functions createEntity(EntityManager em) {
		Functions functions = new Functions().name(DEFAULT_NAME).description(DEFAULT_DESCRIPTION).value(DEFAULT_VALUE)
				.dimensionUse(DEFAULT_DIMENSION_USE).measureUse(DEFAULT_MEASURE_USE);
		return functions;
	}
	...
	public void createFunctions() throws Exception {
		...
		restFunctionsMockMvc.perform(post("/api/functions").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(functionsDTO))).andExpect(status().isCreated());
	...
	public void createFunctionsWithExistingId() throws Exception {
		...
		restFunctionsMockMvc
				.perform(post("/api/functions").contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void getAllFunctions() throws Exception {
		...
		restFunctionsMockMvc.perform(get("/api/functions?sort=id,desc")).andExpect(status().isOk())
	...
	public void getFunctions() throws Exception {
		...
		// Get the functions
		restFunctionsMockMvc.perform(get("/api/functions/{id}", functions.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getNonExistingFunctions() throws Exception {
		// Get the functions
		restFunctionsMockMvc.perform(get("/api/functions/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateFunctions() throws Exception {
		...
		Functions updatedFunctions = functionsRepository.getOne(functions.getId());
		updatedFunctions.name(UPDATED_NAME).description(UPDATED_DESCRIPTION).value(UPDATED_VALUE)
				.dimensionUse(UPDATED_DIMENSION_USE).measureUse(UPDATED_MEASURE_USE);
		FunctionsDTO functionsDTO = functionsMapper.functionsToFunctionsDTO(updatedFunctions);

		restFunctionsMockMvc.perform(put("/api/functions").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(functionsDTO))).andExpect(status().isOk());
	...
	public void deleteFunctions() throws Exception {
		...
		// Get the functions
		restFunctionsMockMvc
				.perform(delete("/api/functions/{id}", functions.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
