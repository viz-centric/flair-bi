...
//        em.flush();
	...
	public void createViews() throws Exception {
		...
		restViewsMockMvc.perform(post("/api/views").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(view))).andExpect(status().isCreated());
	...
	public void createViewsWithExistingId() throws Exception {
		...
		restViewsMockMvc.perform(post("/api/views").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(existingView))).andExpect(status().isBadRequest());
	...
	public void checkViewNameIsRequired() throws Exception {
		...
		restViewsMockMvc.perform(post("/api/views").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(view))).andExpect(status().isBadRequest());
	...
	public void getAllViewsWithPermission() throws Exception {
		...
		restViewsMockMvc.perform(get("/api/views?sort=id,desc")).andExpect(status().isOk())
...
//        viewsRepository.saveAndFlush(view);
		...
		// Get the view
		restViewsMockMvc.perform(get("/api/views/{id}", view.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
...
//            .andExpect(jsonPath("$.image").value(Base64Utils.encodeToString(DEFAULT_IMAGE)));
	...
	public void getNonExistingViews() throws Exception {
		// Get the view
		restViewsMockMvc.perform(get("/api/views/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateViews() throws Exception {
		...
		updatedView.viewName(UPDATED_VIEW_NAME).description(UPDATED_DESCRIPTION).published(UPDATED_PUBLISHED)
				.image(UPDATED_IMAGE).imageContentType(UPDATED_IMAGE_CONTENT_TYPE);

		restViewsMockMvc.perform(put("/api/views").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedView))).andExpect(status().isOk());
	...
	public void deleteViews() throws Exception {
		...
		// Get the view
		restViewsMockMvc.perform(delete("/api/views/{id}", view.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
	...
	public void requestRelease() throws Exception {
		...
		viewService.save(view);
		restViewsMockMvc
				.perform(put("/api/views/{id}/requestRelease", view.getId()).contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void getViewReleases() throws Exception {
		...
		restViewsMockMvc.perform(get("/api/views/{viewId}/releases", viewInserted.getId())).andExpect(status().isOk());
	...
	public void getViewsCount() throws Exception {
		...
		accessControlManager.grantAccess("dash-admin", view.getPermissions());
		restViewsMockMvc.perform(get("/api/views/count")).andExpect(status().isOk())
	...
	public void getRecentlyCreated() throws Exception {
		...
		accessControlManager.grantAccess("dash-admin", view.getPermissions());
		restViewsMockMvc.perform(get("/api/views/recentlyCreated")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getMostPopular() throws Exception {
		...
		accessControlManager.grantAccess("dash-admin", view.getPermissions());
		restViewsMockMvc.perform(get("/api/views/mostPopular")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(recentCreatedView.getId().intValue())))
