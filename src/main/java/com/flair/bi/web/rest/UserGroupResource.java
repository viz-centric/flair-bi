...
/**
 * REST endpoint for managing CRUD operations for
 * {@link com.flair.bi.domain.security.UserGroup}
 */
@RestController
@RequestMapping("/api")
...
public class UserGroupResource {
	 ...
	 * @return The ResponseEntity with status 200 (OK) if new user group has been
	 *         added, or response with status 401(Bad Request) if user group with
	...
	public ResponseEntity<UserGroup> createUserGroup(@Valid @RequestBody UserGroup userGroup)
		...
		if (null != userGroupService.findOne(userGroup.getName())) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "entity exists",
					"User group with this name already exists")).body(null);
		}

		UserGroup result = userGroupService.save(userGroup);
		return ResponseEntity.created(new URI("/api/userGroups/" + result.getName()))
				.headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getName())).body(result);
	}
	 ...
	 * @return The ResponseEntity with status 200 (OK) and updated user group in
	 *         body.
	 */
	...
	public ResponseEntity<UserGroup> updateUserGroup(@Valid @RequestBody UserGroup userGroup) {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, userGroup.getName()))
				.body(result);
	}
	...
	public ResponseEntity<List<String>> getAllUserGroups() {
		return ResponseEntity
				.ok(userGroupService.findAll().stream().map(UserGroup::getName).collect(Collectors.toList()));
	}
	 ...
	 * @return The ResponseEntity with status 200 (OK) and filtered list of user
	 *         groups as body.
	 */
	 ...
	 * @return The ResponseEntity with status 200 (OK) and body containing user
	 ...
	 *
	 * @param name name of user group
	 * @return The response entity with status 200 (OK) if user group was
	...
	/**
	 * POST /userGroups/:name/permissions/:id : adds permission with given id to
	 * user group with given name
	 *
	 * @param name name of userGroup
	 * @param id   id of permission
	 * @return The response entity with status 200 (OK) and valid body returning
	 *         updated user group, or status 404 (Not found) if permission with
	 *         given id was not found or user group with given name was not found.
	 */
	...
	/**
	 * DELETE userGroups/:name/permissions/:id : removes permission with given id
	 * from user group with given name
	 *
	 * @param name name of userGroup
	 * @param id   id of permission
	 * @return the ResponseEntity with status 200 (OK) and valid body body if
	 *         permission was successfully removed, or 404 (Not Found) if permission
	 *         was not found by given id or user group was not found by given name.
	 */
	...
	/**
	 * GET /userGroups/:name/dashboardsMetadata : get all permission metadata for
	 * user group against dashboards and views
	 *
	 * @param name     name of user group
	 * @param pageable the pagination information
	 * @throws URISyntaxException exception
	 * @return the ResponseEntity of status 200 (OK), or status 404(Not found) if
	 *         given user group is not found
	 */
	@GetMapping("/userGroups/{name}/dashboardPermissions")
	@Timed
	public ResponseEntity<List<GranteePermissionReport<UserGroup>>> getDashboardPermissionMetadataUserGroup(
			@PathVariable String name, @ApiParam Pageable pageable) throws URISyntaxException {
		final Page<Dashboard> dashboardPage = dashboardService.findAll(pageable);
		UserGroup userGroup = Optional.ofNullable(userGroupService.findOne(name)).orElseThrow(
				() -> new EntityNotFoundException(String.format("User group with name: %s was not found", name)));

		List<GranteePermissionReport<UserGroup>> body = dashboardPage.getContent().stream()
				.map(x -> x.getGranteePermissionReport(userGroup)).collect(Collectors.toList());

		HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(dashboardPage,
				"/api/userGroups/{name}/dashboardMetadata");
	...
	@Timed
	public ResponseEntity<List<GranteePermissionReport<UserGroup>>> getViewPermissionMetadataUserGroup(
			@PathVariable String name, @PathVariable Long id) {
		UserGroup userGroup = Optional.ofNullable(userGroupService.findOne(name)).orElseThrow(
				() -> new EntityNotFoundException(String.format("User group with name: %s was not found", name)));

		List<GranteePermissionReport<UserGroup>> body = viewService.findByDashboardId(id).stream()
				.map(x -> x.getGranteePermissionReport(userGroup)).collect(Collectors.toList());
	...
	@Timed
	public ResponseEntity<Void> changePermissions(@PathVariable String name,
			@RequestBody List<ChangePermissionVM> changePermissionVMS) {
