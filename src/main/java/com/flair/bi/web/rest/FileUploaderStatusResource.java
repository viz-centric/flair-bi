...
public class FileUploaderStatusResource {
	 ...
	 *         fileUploaderStatusDTO, or with status 400 (Bad Request) if the
	 *         fileUploaderStatus has already an ID
	...
	public ResponseEntity<FileUploaderStatusDTO> createFileUploaderStatus(
			@Valid @RequestBody FileUploaderStatusDTO fileUploaderStatusDTO) throws URISyntaxException {
		log.debug("REST request to save FileUploaderStatus : {}", fileUploaderStatusDTO);
		if (fileUploaderStatusDTO.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("fileUploaderStatus", "idexists",
					"A new fileUploaderStatus cannot already have an ID")).body(null);
		}
	 ...
	 *         fileUploaderStatusDTO, or with status 400 (Bad Request) if the
	 *         fileUploaderStatusDTO is not valid, or with status 500 (Internal
	 *         Server Error) if the fileUploaderStatusDTO couldnt be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/file-uploader-statuses")
	@Timed
	@PreAuthorize("@accessControlManager.hasAccess('FILE_UPLOADER', 'WRITE', 'APPLICATION')")
	public ResponseEntity<FileUploaderStatusDTO> updateFileUploaderStatus(
			@Valid @RequestBody FileUploaderStatusDTO fileUploaderStatusDTO) throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(
				HeaderUtil.createEntityUpdateAlert("fileUploaderStatus", fileUploaderStatusDTO.getId().toString()))
				.body(result);
	}
	 ...
	 *         fileUploaderStatusDTO, or with status 404 (Not Found)
	 */
	...
	public ResponseEntity<FileUploaderStatusDTO> getFileUploaderStatus(@PathVariable Long id) {
		...
		return Optional.ofNullable(fileUploaderStatusDTO).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
	...
	public ResponseEntity<Void> deleteFileUploaderStatus(@PathVariable Long id) {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("fileUploaderStatus", id.toString()))
				.build();
	}
