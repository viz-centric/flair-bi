...
public class FileUploaderStatusResourceIntTest extends AbstractIntegrationTest {
	...
	public void setup() {
		...
		FileUploaderStatusResource fileUploaderStatusResource = new FileUploaderStatusResource(
				fileUploaderStatusService);
	...
	public static FileUploaderStatus createEntity(EntityManager em) {
		FileUploaderStatus fileUploaderStatus = new FileUploaderStatus().fileSystem(DEFAULT_FILE_SYSTEM)
				.fileName(DEFAULT_FILE_NAME).contentType(DEFAULT_CONTENT_TYPE)
				.isFileProcessed(DEFAULT_IS_FILE_PROCESSED).fileLocation(DEFAULT_FILE_LOCATION);
	...
	public void createFileUploaderStatus() throws Exception {
		...
		restFileUploaderStatusMockMvc
				.perform(post("/api/file-uploader-statuses").contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void createFileUploaderStatusWithExistingId() throws Exception {
		...
		FileUploaderStatusDTO existingFileUploaderStatusDTO = fileUploaderStatusMapper
				.fileUploaderStatusToFileUploaderStatusDTO(existingFileUploaderStatus);
	...
	public void updateFileUploaderStatus() throws Exception {
		...
		FileUploaderStatus updatedFileUploaderStatus = fileUploaderStatusRepository.getOne(fileUploaderStatus.getId());
		updatedFileUploaderStatus.fileSystem(UPDATED_FILE_SYSTEM).fileName(UPDATED_FILE_NAME)
				.contentType(UPDATED_CONTENT_TYPE).isFileProcessed(UPDATED_IS_FILE_PROCESSED)
				.fileLocation(UPDATED_FILE_LOCATION);
		FileUploaderStatusDTO fileUploaderStatusDTO = fileUploaderStatusMapper
				.fileUploaderStatusToFileUploaderStatusDTO(updatedFileUploaderStatus);

		restFileUploaderStatusMockMvc
				.perform(put("/api/file-uploader-statuses").contentType(TestUtil.APPLICATION_JSON_UTF8)
						.content(TestUtil.convertObjectToJsonBytes(fileUploaderStatusDTO)))
